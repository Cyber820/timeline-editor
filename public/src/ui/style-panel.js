...
function openAttrPicker(rowId, attrKey) {
  ...
  const cn = (window.attributeLabels && window.attributeLabels[attrKey]) || attrKey;
  ...
  if (attrPickerChoices) {
    attrPickerChoices.destroy();
    attrPickerChoices = null;
  }
  sel.innerHTML = '';

  // 预选：从持久规则里取（需要你已实现 findRule / styleRules）
  const rule = (typeof findRule === 'function') ? findRule(attrKey, rowId) : null;
  const pre  = (rule && Array.isArray(rule.values)) ? rule.values : [];

  // 候选集：复用过滤系统
  const all = (typeof getFilterOptionsForKey === 'function')
    ? (getFilterOptionsForKey(attrKey) || [])
    : (window.allOptions?.[attrKey] || []);

  // 跨行去重：排除“同一属性、其它行”已占用的值，但保留本行已选，避免看不到
  ...
  // 构建 <option>，把本行已选的设为 selected，供 Choices 初始化识别
  options.forEach(v => {
    sel.appendChild(new Option(v, v, /*defaultSelected*/ false, /*selected*/ pre.includes(v)));
  });

  // 初始化 Choices（可搜索+多选标签）
  if (window.Choices) {
    attrPickerChoices = new Choices(sel, {
      removeItemButton: true,
      shouldSort: false,
      searchPlaceholderValue: '搜索…',
      position: 'bottom'
...
function confirmAttrPicker() {
  ...
  if (!rowId || !attrKey || !sel) {
    document.getElementById('attr-picker-window').style.display = 'none';
  ...
  // 终检：与其他行冲突拦截（即使 open 时已过滤，这里再保险一次）
  ...
  // 如仍保留了旧的行内缓存对象，也同步一下（可选）
  if (window.styleRowSelections) {
  ...
  // 读取某一行的“样式键”，用于“具体样式本身”去重：styleType|styleValue
...
// —— 全选：把弹窗当前下拉（已过滤后的）所有选项选中 ——
// 兼容：有 Choices.js 时用实例；没有就设置 <option>.selected
// 全选：把当前下拉里的所有可选项立即设为选中，并同步 Choices UI
// 全选：立即把所有可选值选中，并同步 Choices 的 UI
function selectAllInAttrPicker() {
  ...
  if (attrPickerChoices) { // ✅ 直接用变量，不用 window.*
    // 先清掉已有 token
    attrPickerChoices.removeActiveItems();

    // 保障底层 option 也同步为选中
    Array.from(sel.options).forEach(o => { o.selected = vals.includes(o.value); });

    // 再让 Choices 生成 token（不同版本 API 兼容处理）
    if (vals.length) {
