...
// 修复要点：
// - 事件卡只显示标题（强制仅保留 .event-title）
// - 为每个 .vis-item 写入 .event 与 data-*（attachEventDataAttrs），样式选择器可命中
// - 初次挂载与后续 DOM 变更时都会 re-apply 样式（不使用动态 import）
// - 属性选择器“唯一占用”已保留（同一属性内不同样式行不会复用同一值）
// - 顶部样式按钮挂在过滤按钮右侧；字体含“隶书/幼圆”
...
function buildKvHTML(obj) {
  const kv = [
    ...
    ['标签', Array.isArray(obj.Tag) ? obj.Tag.join('，') : (obj.Tag || '')], ['描述', obj.Description],
...
function normalizeEvent(event, i) {
  ...
  const detailHtml = buildKvHTML({
    title, start, end, EventType, Region, Platform, Company, ConsolePlatform, Tag, Description: Desc, Contributor: Contrib, Status
  });
...
function openStyleEditorFor(attrKey, mapped) {
  ...
  if (typeSel) {
    typeSel.onchange = () => {
      ...
      if (current !== 'none') {
        typeSel.value = 'none'; if (btnConfirm) btnConfirm.disabled = true;
      ...
      if (val !== 'none' && owner && owner !== attrKey) {
        typeSel.value = 'none'; if (btnConfirm) btnConfirm.disabled = true;
        if (hintEl) hintEl.textContent = `“${val}”已绑定到【${owner}】`; return;
      }
...
/* ====================== 关键修复：强制装饰已渲染 DOM ====================== */
// - 为每个 .vis-item 补上 .event
// - 确保 .vis-item-content 内只保留一个 .event-title（避免“显示所有内容”）
// - 写入 data-* 属性（供样式规则匹配）
function forceDecorateRenderedItems(container, dataset) {
  ...
  items.forEach(el => {
    ...
    if (content) {
      ...
      if (!h) {
        ...
        let txt = '';
        try {
          const it = id ? dataset.get(id) : null;
          txt = (it && (it.titleText || it.content)) || (content.textContent || '(无标题)');
        } catch {}
        h.textContent = txt || '(无标题)';
        content.innerHTML = '';
    ...
    try {
      const id = el.getAttribute('data-id');
      if (id) {
        const it = dataset.get(id);
        if (it && typeof attachEventDataAttrs === 'function') {
          attachEventDataAttrs(el, it);
        }
      }
    } catch {}
  });
...
export async function mountTimeline(container, overrides = {}) {
  ...
  try {
    ...
    const baseOptions = {
      ...
      template: (item, element) => {
        ...
        if (host) {
          ...
          try { attachEventDataAttrs(host, item); } catch {}
        }
    ...
    // 初次挂载：补齐 DOM 并应用已有样式
    const reapplyStylesOnce = () => {
      try {
        forceDecorateRenderedItems(container, dataset);
        const saved = getStyleState && getStyleState();
        if (saved && saved.boundTypes) {
          applyStyleState(saved, { selectorBase: '.vis-item.event', titleSelector: '.event-title' });
        }
      } catch (e) { console.warn('[TE] reapply failed', e); }
    };
    reapplyStylesOnce();
    requestAnimationFrame(reapplyStylesOnce);
    timeline.on('changed', reapplyStylesOnce);
    const mo = new MutationObserver(() => {
      if (container.querySelector('.vis-item')) reapplyStylesOnce();
    });
    mo.observe(container, { childList: true, subtree: true });
    window.addEventListener('resize', () => { try { timeline.redraw(); } catch {} reapplyStylesOnce(); });
    ...
    return { timeline, items: dataset, destroy() { try { mo.disconnect(); } catch {} try { timeline.destroy(); } catch {} } };
  } catch (err) {
    console.error(err);
